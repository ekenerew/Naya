    ...
    return (
      <div className="min-h-screen bg-surface-bg flex items-center justify-center p-4">
        <div className="card p-10 max-w-sm w-full text-center">
          ...
          <h2 className="font-display text-2xl font-medium text-obsidian-900 mb-2">Welcome back!</h2>
          <p className="text-obsidian-500 text-sm mb-6">You have been signed in successfully. Redirecting to your dashboard...</p>
        ...
        <div className="relative z-10 flex items-center gap-4">
          {[
            ...
            <div key={i} className="flex items-center gap-1.5 text-white/30 text-xs">
              <b.icon className="w-3.5 h-3.5 text-gold-500" />
              {b.label}
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center justify-center gap-3 mb-8 lg:hidden">
            ...
            <span className="font-display text-2xl font-light text-obsidian-900 tracking-wide">NAYA</span>
          </div>

          {/* Header */}
          <div className="mb-8">
            <h2 className="font-display text-3xl font-medium text-obsidian-900 mb-2">Sign in</h2>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs text-obsidian-400 px-2">or sign in with email</span>
            <div className="flex-1 h-px bg-surface-border" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <Link href="/forgot-password" className="text-xs text-gold-600 hover:text-gold-500 transition-colors">
                  Forgot password?
                </Link>
              </div>
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPass(!showPass)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-obsidian-400 hover:text-obsidian-600 transition-colors">
            ...
            <div className="flex items-center gap-2.5">
              <button type="button" onClick={() => update('remember', !form.remember)}
                className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${form.remember ? 'bg-gold-500 border-gold-500' : 'border-obsidian-300 hover:border-gold-400'}`}>
                {form.remember && <CheckCircle2 className="w-3 h-3 text-obsidian-900" />}
              </button>
              <span className="text-sm text-obsidian-600 cursor-pointer" onClick={() => update('remember', !form.remember)}>
          ...
          <div className="mt-8 pt-6 border-t border-surface-border text-center">
            <p className="text-[10px] text-obsidian-300 leading-relaxed">
              ...
              <Link href="/terms-of-service" className="text-gold-600 hover:underline">Terms of Service</Link>
              {' '}and{' '}
