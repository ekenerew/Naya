  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone, otp, name: userName || undefined }),
      })
      const data = await res.json()
      if (!res.ok) { setError(data.error || 'Invalid code. Please try again.'); return }
      if (data.data?.user?.isNewUser) setIsNewUser(true)
      router.push('/search')
    } catch {
      setError('Network error. Please check your connection.')
    } finally {
        ...
        <div className="w-full max-w-md">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
                        <span className="text-sm font-medium text-obsidian-700">🇳🇬</span>
                        <span className="text-sm text-obsidian-400">+234</span>
                      </div>
                      <input type="tel" className="input-field pl-20 text-sm font-mono"
                        placeholder="080 1234 5678"
                        value={phone} onChange={e => setPhone(e.target.value)} required />
                    </div>
                    <p className="text-xs text-obsidian-400 mt-1.5">Enter your Nigerian mobile number. We'll send an OTP via SMS.</p>
                  </div>

                  <button type="submit" disabled={loading || phone.length < 10}
                    className="btn-primary w-full justify-center gap-2 disabled:opacity-70">
                    {loading ? <><Loader2 className="w-4 h-4 animate-spin" />Sending code...</> : <>Send OTP Code <ArrowRight className="w-4 h-4" /></>}
                  </button>
                </form>
              )}
                ...
                <form onSubmit={handleVerifyOtp} className="space-y-5">
                  <div className="text-center mb-6">
                    ...
                    <p className="text-obsidian-400 text-sm">
                      ...
                      <span className="font-semibold text-obsidian-700">{phone}</span>
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <p className="text-xs text-obsidian-400">Code expires in 5 minutes</p>
          ...
          <div className="mt-6 pt-5 border-t border-surface-border text-center">
            <p className="text-[10px] text-obsidian-300 leading-relaxed">
              ...
              Protected under the Nigeria Data Protection Act 2023.
            </p>
          </div>
        </div>
      </div>
