...
import { Eye, EyeOff, Mail, Lock, ArrowRight, Home, Phone, AlertCircle, Loader2, CheckCircle2, X, ChevronLeft, Shield, Sparkles } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-surface-bg flex">
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-obsidian-900 flex-col justify-between p-12">
        ...
        <div className="relative z-10">
          <h1 className="font-display text-5xl font-light text-white leading-tight mb-6">Nigeria's Most<br/><span className="gold-text">Trusted Property</span><br/>Marketplace</h1>
          <p className="text-white/40 text-base leading-relaxed mb-10 max-w-sm">Sign in to manage listings, track enquiries, and access the full power of Naya's property tools.</p>
        ...
        <div className="relative z-10 flex items-center gap-6">
          {[{icon:Shield,l:'RSSPC Verified'},{icon:CheckCircle2,l:'Encrypted'},{icon:Lock,l:'NDPR Compliant'}].map((b,i)=>(
            <div key={i} className="flex items-center gap-1.5 text-white/30 text-xs"><b.icon className="w-3.5 h-3.5 text-gold-500"/>{b.l}</div>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-obsidian-400 text-sm">Don't have an account?{' '}<Link href="/register" className="text-gold-600 hover:text-gold-500 font-medium">Create one free</Link></p>
          </div>
          ...
          {tab === 'email' && (
            <>
              <button onClick={()=>{ setLoading(true); window.location.href='/api/auth/google' }} disabled={loading}
              ...
              <button onClick={()=>{ setTab('phone'); setError('') }}
                ...
                <Phone className="w-5 h-5 text-emerald-500"/>Continue with Phone Number
              </button>
              ...
              <form onSubmit={signIn} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="email" className="input-field pl-10 text-sm" placeholder="your@email.com" value={email} onChange={e=>setEmail(e.target.value)} required/>
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type={showPass?'text':'password'} className="input-field pl-10 pr-10 text-sm" placeholder="Enter your password" value={password} onChange={e=>setPassword(e.target.value)} required/>
          ...
          {tab === 'phone' && (
            <div>
              <button onClick={()=>{ setTab('email'); setPhoneStep('number'); setOtp(''); setPhone(''); setError('') }}
                className="flex items-center gap-2 text-obsidian-400 hover:text-obsidian-700 text-sm mb-6">
                <ChevronLeft className="w-4 h-4"/>Back
              </button>
              {phoneStep === 'number' && (
                <form onSubmit={sendOtp} className="space-y-5">
                  <div className="text-center mb-4">
                    <div className="w-16 h-16 rounded-2xl bg-emerald-50 flex items-center justify-center mx-auto mb-3"><Phone className="w-8 h-8 text-emerald-500"/></div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="tel" className="input-field pl-24 text-sm font-mono" placeholder="080 1234 5678" value={phone} onChange={e=>setPhone(e.target.value)} required/>
                    </div>
                  </div>
                  <button type="submit" disabled={loading||phone.replace(/\D/g,'').length<10} className="btn-primary w-full justify-center gap-2 disabled:opacity-70">
              ...
              {phoneStep === 'otp' && (
                <form onSubmit={verifyOtp} className="space-y-5">
                  <div className="text-center mb-4">
                    <div className="w-16 h-16 rounded-2xl bg-gold-50 flex items-center justify-center mx-auto mb-3"><Sparkles className="w-8 h-8 text-gold-600"/></div>
                    <h3 className="font-display text-2xl font-medium text-obsidian-900 mb-1">Enter Your Code</h3>
                    <p className="text-obsidian-400 text-sm">Sent to <span className="font-semibold text-obsidian-700">{phone}</span></p>
                  </div>
                  <div>
                    <label className="input-label">6-Digit Code</label>
                    <input type="text" inputMode="numeric" maxLength={6} className="input-field text-center text-2xl font-mono tracking-[0.5em] font-bold" placeholder="000000"
                      value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,'').slice(0,6))} autoFocus required/>
                    <div className="flex justify-between mt-2">
                      <p className="text-xs text-obsidian-400">Expires in 5 minutes</p>
                      <button type="button" onClick={()=>{ setPhoneStep('number'); setOtp(''); setError('') }} className="text-xs text-gold-600 font-medium">Resend</button>
                    </div>
                  </div>
                  <button type="submit" disabled={loading||otp.length!==6} className="btn-primary w-full justify-center gap-2 disabled:opacity-70">
          ...
          <p className="text-center text-sm text-obsidian-400 mt-6">
            Don't have an account?{' '}<Link href="/register" className="text-gold-600 font-medium">Create account</Link>
          </p>
          <p className="text-center text-[10px] text-obsidian-300 mt-4">
