...
set -e
cd ~/naya
echo "=== Writing all fix files directly ==="

# 1. Fix Footer links
sed -i "s|href: '/legal/privacy'|href: '/privacy'|g" components/layout/Footer.tsx
sed -i "s|href: '/legal/terms'|href: '/terms'|g" components/layout/Footer.tsx
echo "✓ Footer links fixed"

# 2. Ensure terms page has 'use client'
if ! head -1 app/terms/page.tsx | grep -q "use client"; then
  sed -i "1s/^/'use client'\n/" app/terms/page.tsx
fi
if ! head -1 app/privacy/page.tsx | grep -q "use client"; then
  sed -i "1s/^/'use client'\n/" app/privacy/page.tsx
fi
echo "✓ Terms/Privacy use client confirmed"

# 3. Create terms-of-service that just re-exports (works in Next.js 15+)
...
# 4. Create privacy-policy that just re-exports
...
# 5. Fix currency dropdown
cat > components/features/DiasporaMode.tsx << 'DIASPORA'
'use client'
...
echo "✓ DiasporaMode fixed"

# 6. Git push
echo "=== Git status ==="
git status

git add components/layout/Footer.tsx app/terms/page.tsx app/privacy/page.tsx app/terms-of-service/page.tsx app/privacy-policy/page.tsx components/features/DiasporaMode.tsx

git status

git commit -m "Fix: footer links, terms/privacy pages, currency dropdown"
